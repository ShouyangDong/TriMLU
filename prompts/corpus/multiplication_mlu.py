...
performance on parallel with mlu.
...
def matmul_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        if EVEN_K:
            a = tl.load(a_ptrs, mask=(offs_m < M)[:, None], other=0.0)
            b = tl.load(b_ptrs, mask=(offs_n < N)[None, :], other=0.0)
        else:
            k_remaining = K - k * BLOCK_SIZE_K
            a = tl.load(
                a_ptrs,
                mask=(offs_k[None, :] < k_remaining) & (offs_m < M)[:, None],
                other=0.0,
                cache_modifier=".cg",
            )
            b = tl.load(
...
TORCH_HAS_FP8 = hasattr(torch,
                        "float8_e5m2") and torch.mlu.get_device_capability()[0] >= 6
if __name__ == "__main__":
    torch.manual_seed(0)
    a = torch.randn((512, 512), device='mlu', dtype=torch.float16)
    b = torch.randn((512, 512), device='mlu', dtype=torch.float16)
    ...
    if TORCH_HAS_FP8:
        ...
        a = torch.randn((512, 512), device="mlu", dtype=torch.float16)
        b = torch.randn((512, 512), device="mlu", dtype=torch.float16)
        a = a.cpu().to(torch.float8_e5m2).view(torch.int8).to("mlu").view(
            torch.float8_e5m2)
        # pre-transpose b for efficiency.
        b = b.cpu().to(torch.float8_e5m2).view(torch.int8).to("mlu").view(
            torch.float8_e5m2)
        triton_output = matmul(a, b)
        torch_output = torch.matmul(
            a.view(torch.int8).cpu().view(torch.float8_e5m2).to(torch.float16),
            b.view(torch.int8).cpu().view(torch.float8_e5m2).to(
                torch.float16)).to('mlu')
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~
#
# We can now compare the performance of our kernel against that of mlu. Here we focus on square matrices,
...
for fp8_inputs in [False, True]:
    ...
    configs.append(
        triton.testing.Benchmark(
            ...
            x_vals=[128 * i
                    for i in range(2, 33)],  # Different possible values for `x_name`
            line_arg=
            "provider",  # Argument name whose value corresponds to a different line in the plot
            # Possible values for `line_arg`
            # Don't compare to cublas for fp8 cases as torch.matmul doesn't support fp8 at the moment.
            line_vals=["triton"]
            if fp8_inputs else ['torch', "triton"],  # Label name for the lines
            line_names=["Triton"] if fp8_inputs else ['torch', "Triton"],  # Line styles
            styles=[("green", "-"), ("blue", "-")],
            ylabel="TFLOPS",  # Label name for the y-axis
            plot_name="matmul-performance-" +
            ("fp16" if not fp8_inputs else
             "fp8"),  # Name for the plot, used also as a file name for saving the plot.
