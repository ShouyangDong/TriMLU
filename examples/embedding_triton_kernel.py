...
def embedding_wrapper(
    input_ids, weight: torch.Tensor, vob_start_id, vob_end_id
):
...
def test_embedding_correctness():
    ...
    for seq_len in configs:
        input_ids = torch.randint(0, vocab_size + 10, (seq_len,), dtype=torch.int32, device="mlu")
        weight = torch.randn(vob_end_id - vob_start_id, embedding_dim, dtype=torch.float32, device="mlu")
        
        # Triton 结果
        triton_out = embedding_wrapper(input_ids, weight, vob_start_id, vob_end_id)
        
        # Torch 参考结果 (手动实现逻辑以匹配 vob 范围限制)
        torch_out = torch.zeros((seq_len, embedding_dim), dtype=torch.float32, device="mlu")
...
def benchmark_embedding():
    ...
    def benchmark(SEQ_LEN, D, vob_start, vob_end, provider):
        input_ids = torch.randint(vob_start, vob_end, (SEQ_LEN,), dtype=torch.int32, device='mlu')
        weight = torch.randn(vob_end - vob_start, D, dtype=torch.float32, device='mlu')
        
        if provider == 'torch':
            # 简化 Torch 版测试，仅包含核心 embedding
            ms = triton.testing.do_bench(lambda: torch.nn.functional.embedding(input_ids, weight))
        if provider == 'triton':
            ms = triton.testing.do_bench(lambda: embedding_wrapper(input_ids, weight, vob_start, vob_end))
